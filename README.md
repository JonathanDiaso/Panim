...
content/
  ...
  audio-host.js     WHERE THE AUDIO IS SERVED FROM. '' = this origin, or a bucket URL.
                    Read by js/player.js and by sw.js (importScripts).
  audio-manifest.js durations + music offsets (source of truth for run times)
  audio-manifest-es.js  GENERATED by tools/build-spanish-audio.py — the Spanish
                    edition's run times, sizes, titles. Never hand-edit.
  es/chNN.json      GENERATED by tools/build-spanish-text.py — the Spanish words,
                    keyed by the ENGLISH block ids. Fetched by js/spanish-text.js
                    only when Spanish is chosen. Never hand-edit.
...
js/   render.js (DOM; also BUILDS the contents, the thread, the lexicon and the
      ...
      spanish-text.js (swaps the words in place when Spanish is chosen)
